...
Per the protocol described in \autoref{subsec:protocol}, I answer this question based on manual analysis:
I randomly selected 1 commit per project, and analyze the relevance of the selected tests for amplification.

In order to give an intuition of what are the characteristics of the test selection for amplification to be relevant, let us look at an example.
...
This ground-truth test method is also in the test class \texttt{com.github.mustachejava.InterpreterTest}, this is considered as a success.
...
As before, I compare it to the developer's test. 
...
the variable names, the explanatory comments and even the vertical space formatting are impossible to achieve with \DCIA and makes the human test clearly of better quality but also longer to write. % above when she has the time to write a good test. 
%
...
In practice, I would recommend to set a time budget (\eg at most one hour per pull-request).
...
While this is not a proof that \DCI would never produce such confusing test methods, It encourage to be confident in the soundness of our implementation.
...
An internal threat is the potential bugs in the implementation of DCI. However, it is heavily tested, with JUnit test cases to mitigate this threat.

% benchmark
In the benchmark, there are 60 commits. The result may be not be generalizable to all programs. 
But real and diverse applications from GitHub have been carefully selected, all having a strong test suite. 
The benchmark reflects real programs, and the results can be trust.
