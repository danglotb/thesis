...
%For input points within $I_{bug}$ whose behaviors have been changed by a patch, the patch can either correctly or incorrectly change the original buggy behaviors.
%In the former case, input points within $I_{bug}$ are denoted $I_{patch=}$, \ie the newly behaviors of these input points are still incorrect.
%In the latter case, input points within $I_{bug}$ are denoted $I_{patch{\cmark}}$, \ie the newly behaviors have been correctly changed by a patch. 
%If the patch changes the behaviors of input points within $I_{correct}$, then the original correct behaviors of these input points will undesirably become incorrect and we use $I_{patch{\xmark}}$ to denote these input points within $I_{correct}$ broken by the patch.
%One can note, the union of $I_{patch=}$, $I_{patch{\cmark}}$ and $I_{patch{\xmark}}$ makes up $I_{patch}$.
